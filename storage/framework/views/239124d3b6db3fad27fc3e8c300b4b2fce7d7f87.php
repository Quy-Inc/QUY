		...
		<div id="content" style="padding-top: 0px;">
		    <div class="row">
		        <?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'agama'):''; ?>
		        ...
		        <section class="panel">
		            ...
		            <div class="panel-body">
		                <table class="table table-striped" id="tagama">
		                    <thead>
		                        <tr>
		                            <th>Agama</th>
		                            <th>Action</th>
...
$(function() {
    ...
    $('#tagama').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo url("/ajax/agama/getagama"); ?>',
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $('.switch').change(function(event) {
                ...
                $.ytLoad();
                var uri = "<?php echo url('ajax/agama/updateagamad'); ?>";
                $.ajax({
                    url: uri,
                    data: {
                        'data': dataid,
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click', '.showmodaldelagama', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    $modal.find(".modal-title").html("Delete Agama");
    $modal.find(".modal-body").html("Are you sure want to delete agama data?");
    $modal.find(".modal-footer").html(
        '<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelagama" valueajax="' +
...
//Deleted Data
$(document).on('click', '.yesdelagama', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    ...
    $.ytLoad();
    var uri = "<?php echo url('ajax/agama/deleteagamad'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': valueid,
