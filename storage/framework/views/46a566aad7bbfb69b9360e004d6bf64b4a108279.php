...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'merchant'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="tmerchant" >
								<thead>
										<tr>
											<th>ID Merchant</th>
											<th>Merchant Name</th>
...
$(function() {
	...
	$('#tmerchant').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/merchant/getmerchant"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/merchant/updatemerchantd'); ?>";
				$.ajax({
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click','.showmodaldelmerchant', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Merchant");
	$modal.find(".modal-body").html("Are you sure want to delete merchant data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelmerchant" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelmerchant', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/merchant/deletemerchantd'); ?>";
	$.ajax({
