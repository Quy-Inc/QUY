		...
		<div id="content" style="padding-top: 0px;">
		    <div class="row">
		        <?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'psbTA'):''; ?>
		        ...
		        <section class="panel">
		            ...
		            <div class="panel-body">
		                <table class="table table-striped" id="tpsbTA">
		                    <thead>
		                        <tr>
		                            <th>No</th>
		                            <th>Witel</th>
...
$(function() {
    ...
    $('#tpsbTA').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo url("/ajax/psbTA/getpsbTA"); ?>',
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $('.switch').change(function(event) {
                ...
                $.ytLoad();
                var uri = "<?php echo url('ajax/psbTA/updatepsbTAd'); ?>";
                $.ajax({
                    url: uri,
                    data: {
                        'data': dataid,
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click', '.showmodaldelpsbTA', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    $modal.find(".modal-title").html("Delete PsbTA");
    $modal.find(".modal-body").html("Are you sure want to delete psbTA data?");
    $modal.find(".modal-footer").html(
        '<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelpsbTA" valueajax="' +
...
//Deleted Data
$(document).on('click', '.yesdelpsbTA', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    ...
    $.ytLoad();
    var uri = "<?php echo url('ajax/psbTA/deletepsbTAd'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': valueid,
