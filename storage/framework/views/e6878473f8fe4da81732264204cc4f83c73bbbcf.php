...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'kebutuhankhusus'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="tkebutuhankhusus" >
								<thead>
										<tr>
											<th>Tipe</th>
											<th>Action</th>
...
$(function() {
	...
	$('#tkebutuhankhusus').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/kebutuhankhusus/getkebutuhankhusus"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/kebutuhankhusus/updatekebutuhankhususd'); ?>";
				$.ajax({
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click','.showmodaldelkebutuhankhusus', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Kebutuhankhusus");
	$modal.find(".modal-body").html("Are you sure want to delete kebutuhankhusus data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelkebutuhankhusus" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelkebutuhankhusus', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/kebutuhankhusus/deletekebutuhankhususd'); ?>";
	$.ajax({
