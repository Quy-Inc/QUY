...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'menus'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="tmenus" >
								<thead>
										<tr>
											<th></th>
										</tr>
...
$(function() {
	...
	$('#tmenus').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/menus/getmenus"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/menus/updatemenusd'); ?>";
				$.ajax({
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click','.showmodaldelmenus', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Menus");
	$modal.find(".modal-body").html("Are you sure want to delete menus data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelmenus" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelmenus', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/menus/deletemenusd'); ?>";
	$.ajax({
