		...
		<div id="content" style="padding-top: 0px;">
		    <div class="row">
		        <?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'pendaftaran'):''; ?>
		        ...
		        <section class="panel">
		            ...
		            <div class="panel-body">
		                <table class="table table-striped" id="tpendaftaran">
		                    <thead>
		                        <tr>
...
$(function() {
    $('#tpendaftaran').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo url("/ajax/pendaftaran/getpendaftaran"); ?>',
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $('.switch').change(function(event) {
                ...
                $.ytLoad();
                var uri = "<?php echo url('ajax/pendaftaran/updatependaftarand'); ?>";
                $.ajax({
                    url: uri,
                    data: {
                        'data': dataid,
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click', '.showmodaldelpendaftaran', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    $modal.find(".modal-title").html("Delete Pendaftaran");
    $modal.find(".modal-body").html("Are you sure want to delete pendaftaran data?");
    $modal.find(".modal-footer").html(
        '<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelpendaftaran" valueajax="' +
...
//Deleted Data
$(document).on('click', '.yesdelpendaftaran', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    ...
    $.ytLoad();
    var uri = "<?php echo url('ajax/pendaftaran/deletependaftarand'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': valueid,
...
// Detail siswa
$(document).on('click', '.showmodaldetailpendaftaran', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var idpendaftaran = $(this).attr('valueajax');
    var $modaldetail = $('#md-full-width');
    $modaldetail.find(".modal-body").html("Loading");
    var uri = "<?php echo url('pendaftaran/detailpendaftaran'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': idpendaftaran,
