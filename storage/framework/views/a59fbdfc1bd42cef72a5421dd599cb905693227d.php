...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'webblog'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="twebblog" >
								<thead>
										<tr>
											<th>Title</th>
											<th>Body</th>
...
$(function() {
	...
	$('#twebblog').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/webblog/getwebblog"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/webblog/updatewebblogd'); ?>";
				$.ajax({
...
$(document).on('click','.showmodaldel', function(e){
	...
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Webblog");
	$modal.find(".modal-body").html("Are you sure want to delete webblog data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelwebblog" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelwebblog', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/webblog/deletewebblogd'); ?>";
	$.ajax({
