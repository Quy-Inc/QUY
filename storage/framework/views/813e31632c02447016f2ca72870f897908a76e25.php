...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'kabupaten'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="tkabupaten" >
								<thead>
										<tr>
											<th>Provinsi</th>
...
$(function() {
	...
	$('#tkabupaten').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/kabupaten/getkabupaten"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/kabupaten/updatekabupatend'); ?>";
				$.ajax({
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click','.showmodaldelkab', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Kabupaten");
	$modal.find(".modal-body").html("Are you sure want to delete kabupaten data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelkabupaten" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelkabupaten', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/kabupaten/deletekabupatend'); ?>";
	$.ajax({
