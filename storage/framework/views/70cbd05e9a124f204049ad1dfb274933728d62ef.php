		...
		<div id="content" style="padding-top: 0px;">
		    <div class="row">
		        <?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'sto'):''; ?>
		        ...
		        <section class="panel">
		            ...
		            <div class="panel-body">
		                <table class="table table-striped" id="tsto">
		                    <thead>
		                        <tr>
		                            <th>Kode STO</th>
		                            <th>Nama STO</th>
...
$(function() {
    ...
    $('#tsto').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo url("/ajax/sto/getsto"); ?>',
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $('.switch').change(function(event) {
                ...
                $.ytLoad();
                var uri = "<?php echo url('ajax/sto/updatestod'); ?>";
                $.ajax({
                    url: uri,
                    data: {
                        'data': dataid,
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click', '.showmodaldelsto', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    $modal.find(".modal-title").html("Delete Sto");
    $modal.find(".modal-body").html("Are you sure want to delete sto data?");
    $modal.find(".modal-footer").html(
        '<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelsto" valueajax="' +
...
//Deleted Data
$(document).on('click', '.yesdelsto', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    ...
    $.ytLoad();
    var uri = "<?php echo url('ajax/sto/deletestod'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': valueid,
