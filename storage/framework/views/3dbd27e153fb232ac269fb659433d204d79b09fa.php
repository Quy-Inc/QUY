...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'kecamatan'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="tkecamatan" >
								<thead>
										<tr>
											<th>ID Kecamatan</th>
											<th>Kabupaten</th>
...
$(function() {
	...
	$('#tkecamatan').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/kecamatan/getkecamatan"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/kecamatan/updatekecamatand'); ?>";
				$.ajax({
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click','.showmodaldelkec', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Kecamatan");
	$modal.find(".modal-body").html("Are you sure want to delete kecamatan data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelkecamatan" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelkecamatan', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/kecamatan/deletekecamatand'); ?>";
	$.ajax({
