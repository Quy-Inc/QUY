		...
		<div id="content" style="padding-top: 0px;">
		    <div class="row">
		        <?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'tatelkom'):''; ?>
		        ...
		        <section class="panel">
		            ...
		            <div class="panel-body">
		                <table class="table table-striped" id="ttatelkom">
		                    <thead>
		                        <tr>
		                            <th>Nama</th>
		                            <th>Keterangan</th>
...
$(function() {
    ...
    $('#ttatelkom').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo url("/ajax/tatelkom/gettatelkom"); ?>',
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $('.switch').change(function(event) {
                ...
                $.ytLoad();
                var uri = "<?php echo url('ajax/tatelkom/updatetatelkomd'); ?>";
                $.ajax({
                    url: uri,
                    data: {
                        'data': dataid,
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click', '.showmodaldeltatelkom', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    $modal.find(".modal-title").html("Delete Tatelkom");
    $modal.find(".modal-body").html("Are you sure want to delete tatelkom data?");
    $modal.find(".modal-footer").html(
        '<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdeltatelkom" valueajax="' +
...
//Deleted Data
$(document).on('click', '.yesdeltatelkom', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    ...
    $.ytLoad();
    var uri = "<?php echo url('ajax/tatelkom/deletetatelkomd'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': valueid,
