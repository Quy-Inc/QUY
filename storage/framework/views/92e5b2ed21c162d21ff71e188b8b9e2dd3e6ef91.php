...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'witel'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="twitel" >
								<thead>
										<tr>
											<th>Nama Witel</th>
											<th>Alamat</th>
...
$(function() {
	...
	$('#twitel').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/witel/getwitel"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/witel/updatewiteld'); ?>";
				$.ajax({
...
$(document).on('click','.showmodaldel', function(){
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Hapus Data Witel");
	$modal.find(".modal-body").html("Anda yakin akan menghapus Witel ini?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelwitel" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelwitel', function(){
	var valueid = $(this).attr('valueajax');
	var $modal = $('#md-normal');

	$.ytLoad();
	var uri = "<?php echo url('ajax/witel/deletewiteld'); ?>";
	$.ajax({
