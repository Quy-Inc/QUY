		...
		<div id="content" style="padding-top: 0px;">
		    <div class="row">
		        <?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'pek'):''; ?>
		        ...
		        <section class="panel">
		            ...
		            <div class="panel-body">
		                <table class="table table-striped" id="tpek">
		                    <thead>
		                        <tr>
		                            <th>Kode Pek</th>
		                            <th>Nama Pek</th>
...
$(function() {
    ...
    $('#tpek').DataTable({
        processing: true,
        serverSide: true,
        ajax: '<?php echo url("/ajax/pek/getpek"); ?>',
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $('.switch').change(function(event) {
                ...
                $.ytLoad();
                var uri = "<?php echo url('ajax/pek/updatepekd'); ?>";
                $.ajax({
                    url: uri,
                    data: {
                        'data': dataid,
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click', '.showmodaldelpek', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    $modal.find(".modal-title").html("Delete Pek");
    $modal.find(".modal-body").html("Are you sure want to delete pek data?");
    $modal.find(".modal-footer").html(
        '<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelpek" valueajax="' +
...
//Deleted Data
$(document).on('click', '.yesdelpek', function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    var valueid = $(this).attr('valueajax');
    ...
    $.ytLoad();
    var uri = "<?php echo url('ajax/pek/deletepekd'); ?>";
    $.ajax({
        url: uri,
        data: {
            'data': valueid,
