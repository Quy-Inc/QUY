...
<div id="content" style="padding-top: 0px;">
	<div class="row">
<?php echo (isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'provinsi'):''; ?>
				...
				<section class="panel">
						...
						<div class="panel-body">
							<table class="table table-striped" id="tprovinsi" >
								<thead>
										<tr>
											<th>ID Provinsi</th>
											<th>Nama Provinsi</th>
...
$(function() {
	...
	$('#tprovinsi').DataTable({
		processing: true,
		serverSide: true,
		ajax: '<?php echo url("/ajax/provinsi/getprovinsi"); ?>',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "<?php echo url('ajax/provinsi/updateprovinsid'); ?>";
				$.ajax({
...
//Delete Confirm
var $modal = $('#md-normal');
$(document).on('click','.showmodaldelprov', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	$modal.find(".modal-title").html("Delete Provinsi");
	$modal.find(".modal-body").html("Are you sure want to delete provinsi data?");
	$modal.find(".modal-footer").html('<button type="button" class="btn btn-inverse" data-dismiss="modal" aria-hidden="true">Tutup</button><button type="button" class="btn btn-danger  yesdelprovinsi" valueajax="'+valueid+'" >Setuju</button>');
	$modal.modal('show');
});


//Deleted Data
$(document).on('click','.yesdelprovinsi', function(e){
	e.preventDefault();
	e.stopPropagation();
	e.stopImmediatePropagation();
	var valueid = $(this).attr('valueajax');
	...
	$.ytLoad();
	var uri = "<?php echo url('ajax/provinsi/deleteprovinsid'); ?>";
	$.ajax({
