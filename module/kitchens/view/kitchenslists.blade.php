...
@endphp
	<div class="row">
{!!(isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'kitchens'):''!!}
				<section class="panel">
...
$(function() {
	...
	$('#tkitchens').DataTable({
		...
		serverSide: true,
		ajax: {
            url:'{!!url("/ajax/kitchens/getkitchens")!!}',
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "{!!url('ajax/kitchens/updatekitchensd')!!}";
				$.ajax({
...
$(document).on('click','.showmodaldetailkitchens', function(e){
	...
	var uri = "{!!url('kitchens/detailkitchens')!!}";
	$.ajax({
...
$(document).on('click','.yesdelkitchens', function(e){
	...
	$.ytLoad();
	var uri = "{!!url('ajax/kitchens/deletekitchensd')!!}";
	$.ajax({
