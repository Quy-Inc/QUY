...
$(function() {
	...
	$('#tstores').DataTable({
		...
		serverSide: true,
		ajax: {
            url:'{!!url("/ajax/stores/getstores")!!}',
            data:{id_merchants:'{!!$idMerchant!!}',id_venues:'{!!$idVenue!!}'},
        },
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.switch').change(function (event) {
				...
				$.ytLoad();
				var uri = "{!!url('ajax/stores/updatestoresd')!!}";
				$.ajax({
...
$(document).on('click','.showmodaldetailstores', function(e){
	...
	var uri = "{!!url('stores/detailstores')!!}";
	$.ajax({
...
$(document).on('click','.yesdelstores', function(e){
	...
	$.ytLoad();
	var uri = "{!!url('ajax/venues/deletestoresd')!!}";
	$.ajax({
