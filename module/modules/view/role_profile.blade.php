...
$(function() {
	...
	function sendrole(role,idprofile,slugaction,statusrole){
		var hrefattr = '{!!url("/ajax/modules/sendrole")!!}';
		$.ytLoad();
		$.post(hrefattr,({"idprofile":idprofile,"module":role,"slugaction":slugaction,"statusrole":statusrole,"_token":"{!!csrf_token()!!}"}), function( response, status, xhr ) {
		  if ( status == "error" ) {
	...
	$('#role-table').DataTable({
		...
		serverSide: true,
		ajax: {
			url:'{!!url("/ajax/modules/getprofilesdata")!!}',
			data: function (d) {
				d.slug = '{!!$slug!!}';
			}
		},
