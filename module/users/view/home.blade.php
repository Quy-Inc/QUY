...
<div id="content" style="padding-top: 0px;">
	<div class="row">
{!!(isset($content))?Ozn::getSubMenuIcon($content->sub_menu,$content->module_slug,'users'):''!!}
				<section class="panel">
...
$(function() {
        $('#users-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{!!url("/ajax/users/getuserdata")!!}',
			...
			"fnDrawCallback": function (oSettings) {
				...
				$('.switch').change(function (event) {
					...
					$.ytLoad();
					var uri = "{!!url('ajax/users/updatestatususersdata')!!}";
					$.ajax({
...
function deletedata(valueid){
	...
	$.ytLoad();
	var uri = "{!!url('ajax/users/deleteusersdata')!!}";
	$.ajax({
